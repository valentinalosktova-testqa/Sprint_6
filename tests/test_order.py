...
def test_order_scooter(driver, order_data):
    ...
    order_page.click_order_button()  # Далее

    order_page.fill_rental_form(                  # вторая форма
        order_data["date"],
        order_data["period"],
        order_data["color"],
        order_data["comment"]
    )
    order_page.click_final_order_button()  # Заказать
